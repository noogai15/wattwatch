...
class ImageUtils {
  ...
  static img.Image upscaleImage(img.Image image, double scaleFactor) {
    ...
    final nWidth = (image.width * scaleFactor).round();
    final nHeight = (image.height * scaleFactor).round();

    // Bild skalieren
    final dstImage = img.copyResize(image,
        width: nWidth, height: nHeight, interpolation: img.Interpolation.cubic);
